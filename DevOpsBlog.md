...
## 提交阶段流水线
1. 目标：进行代码构建和质量扫描，提高代码质量
2. 触发条件：开发push代码到develop分支
3. 流程设计：
  - 拉去代码
  - 代码编译
  - 单元测试
...
# 使用SaltStack 
1. 先做database pipeline 后做code pipeline
2. 将所有的一切纳入版本控制中
...
- 蓝绿部署 
  -- 部署环境蓝，绿环境，两个环境，蓝是1.0版本用户正在使用。绿版本是新版本，将用户环境逐渐转换到绿环境。如果绿环境有问题立即转换到绿环境

- 金丝雀发布
  -- 就分新老版本，将新版本发布给一小部分用户，看反应如果，如果效果好在全局发布。尽量避免故障造成的重大影响。
...
删除salt
$ yum remove salt-master salt-minion 
$ rm -rf /etc/salt
